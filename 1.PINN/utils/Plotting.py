...
def plot_pts(Xint, Xbnd, axisScale=None, title="Boundary and interior collocation points"):
    """
    ...
    if axisScale == 1:
        xmin = min(Xint[:, 0].min(), Xbnd[:, 0].min())
        xmax = max(Xint[:, 0].max(), Xbnd[:, 0].max())
        ymin = min(Xint[:, 1].min(), Xbnd[:, 1].min())
        ymax = max(Xint[:, 1].max(), Xbnd[:, 1].max())
        plt.xlim(xmin, xmax)
        plt.ylim(ymin, ymax)
