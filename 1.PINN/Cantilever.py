...
For this example:
   ...
   Dirichlet boundary conditions for x=0:
    ...
    and parabolic traction at x=8
           p(x,y) = P*(y^2 - y*W)/(2*I)
    where P=2 is the maximum traction
...
xPhys, yPhys, Wint = geomDomain.getQuadIntPts(numPtsU, numPtsV, 4)
data_type = "float64"

Xint = np.concatenate((xPhys, yPhys), axis=1).astype(data_type)
Yint = np.zeros_like(Xint).astype(data_type)

# prepare boundary points in the format Xbnd = [Xcoord, Ycoord, dir] and
...
print("Pressure = ", pressure, "Elasticity = ", model_data["E"])
...
xPhysTest, yPhysTest, Wint = geomDomain.getQuadIntPts(numPtsUTest, numPtsVTest, 4)
...
with open(('loss1.txt'), "w") as file:
    for value in pred_model.adam_loss_hist:
        file.write(str(value.numpy()) + "\n")

with open(('loss2.txt'), "w") as file:
    for value in loss_func.history:
        file.write(str(value.numpy()) + "\n")
