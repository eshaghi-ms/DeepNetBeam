...
PATH_t = "database/traction_10x128.npy"
PATH_m = "database/material_10x32.npy"
PATH_d = 'database/disp2D_10x128x32x2.npy'
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(traction_train, material_train,
                                  disp2D_train), batch_size=batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(traction_test, material_test, disp2D_test),
                                  batch_size=batch_size, shuffle=False)

# model
model = FNO2d(modes, modes, width, model_data).to(device)
...
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(traction_test, material_test,
...
def test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, porositydist):
    ...
    elif porositydist =="dist3":
        elasticity_modulus = Emod*(1-1/e0-1/e0*(2/np.pi*(np.sqrt(1-e0)-2/np.pi+1))**2)
    ...
    t = time.time()
    disp_net = model(f_tensor, E_tensor)
    t_net = time.time() - t
...
f_name = "sin-dist1"
f = 0.25*np.sin(x*4*np.pi/l)+0.25
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist1")
...
# ---------------------------
# testing f=(1/4)cos((4*pi/l)*x)+1/4
f_name = "cos-dist1"
f = 0.25*np.sin(x*4*np.pi/l)+0.25
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist1")
...
f_name = "exponential-dist1"
f = -(np.exp(-x)-1)*(np.exp(l-x)-1)/l**2
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist1")

f_name = "exponential-dist2"
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist2")

# ---------------------------
# testing f=reciprocal
f_name = "reciprocal-dist1"
f = -(x*(x-l))/(x+l/2)**2
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist1")

f_name = "reciprocal-dist2"
test_result(f_name, f, Emod, nu, e0, beam_length, beam_width, "dist2")
